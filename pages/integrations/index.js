...
const SHOPIFY_CLIENT_ID = "ca4d14dd94fd8e69c07cd4491858361d";
    ...
    if (window.location.search) {
      ...
      if (params.get("shop")) {
        window.location = `https://${params.get(
          "shop",
        )}.myshopify.com/admin/oauth/authorize?client_id=${SHOPIFY_CLIENT_ID}&redirect_uri=${REDIRECT_URL}&scope=${SHOPIFY_SCOPES}`;
      }
              ...
              <button
                onClick={() => {
                  if (integration.type === "pipedrive") {
                    paragon.connect("pipedrive", {
                      overrideRedirectUrl:
                        "https://w4tdtq-3000.csb.app/integrations/pipedrive",
                    });
